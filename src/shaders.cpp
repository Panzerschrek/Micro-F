...
"const float c_cloud_destiny_border= 0.53;"
"const float c_cloud_border= 1.0 / 32.0;"

"float CloudFunc( vec3 xyz )"
"{"
	"float depth_k= float(xyz.z - c_clouds_height) / c_clouds_depth;"
	"float destiny= depth_k * ( 4.0 - 4.0 * depth_k );"
	"return destiny * OcatveNoise( xyz * 0.25 + vec3(2048,2048,2048), 5 );"
"}"
...
"{"
	...
	"{"
		...
		"{"
		"if(!is_first_pos){touch_pos=pos;is_first_pos=true;}"
			"float mul= 1.0 - ( val - c_cloud_destiny_border ) * 2.0;"
			"if( mul < 0.0) mul= 0.0;"
			"sky_k*= mul;"
