	...
	, settings_(*settings)
{
	...
	static const char* const powerups_shader_uniforms[]= { "mat", "nmat", "mmat", /*frag*/ "c", "sun", "sl", "alcube", "trans" };
	...
	{ // clouds generation shader
		...
		static const char* const clouds_gen_uniforms_names[]= {
			"mat", "sl", "al", "sun", "inv_tex_size", "horizont" };
		sky_clouds_data_.clouds_gen_shader.FindUniforms( clouds_gen_uniforms_names, sizeof(clouds_gen_uniforms_names) / sizeof(char*) );

		// clouds downscale shader
		sky_clouds_data_.clouds_downscale_shader.Create( mf_Shaders::clouds_downscale_shader_v, mf_Shaders::clouds_downscale_shader_f );
		sky_clouds_data_.clouds_downscale_shader.FindUniform( "tex" );
	}
	...
	{ // clouds mesh
		...
		unsigned int shift;
		shift= (char*)vert.pos - (char*)&vert;
		sky_clouds_data_.clouds_dome_vbo.VertexAttrib( 0, 3, GL_FLOAT, false, shift );
	}
	{ // stars
		mf_Rand randomizer;
...
{
	static const float c_horizont_height= 0.1f;
	{ // gen skydome mesh
	...
	static const unsigned int c_clouds_quality_size[ mf_Settings::LastQuality ]= { 1024, 1536, 2048 };
	unsigned int generation_texture_size= c_clouds_quality_size[ settings_.sky_quality ];
	sky_clouds_data_.texture_size= generation_texture_size / 2;
	...
	for( unsigned int i= 0; i< 6; i++ )
		glTexImage2D(
			...
			sky_clouds_data_.texture_size, sky_clouds_data_.texture_size, 0,
			GL_RGBA, GL_UNSIGNED_BYTE, NULL );

	glGenTextures( 1, &tex_id );
	glBindTexture( GL_TEXTURE_2D, tex_id );
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA8,
		generation_texture_size, generation_texture_size, 0,
		GL_RGBA, GL_UNSIGNED_BYTE, NULL );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
	...
	float inv_tex_size[3]={ 1.0f / float(generation_texture_size), 1.0f / float(generation_texture_size), 0.0f };
	...
	{
		glBindFramebuffer( GL_FRAMEBUFFER, fbo_id );
		glViewport( 0, 0, generation_texture_size, generation_texture_size );
		...
		if( i != 5 ) // do not draw lower side
		{
			...
			for( unsigned int x= 0; x< 4; x++ )
				...
				{
					glScissor( generation_texture_size * x / 4, generation_texture_size * y / 4,
						generation_texture_size / 4, generation_texture_size / 4 );
		...
		sky_clouds_data_.clouds_downscale_shader.Bind();
		glBindTexture( GL_TEXTURE_2D, tex_id );
		sky_clouds_data_.clouds_downscale_shader.UniformInt( "tex", 0 );
	...
	glDeleteFramebuffers( 1, &cubemap_fbo_id );
	...
	sky_clouds_data_.clouds_cubemap_tex_id= cubemap_tex_id;
}
...
void mf_Renderer::PreparePowerupsModels()
{
	unsigned int total_vertex_count= 0;
	unsigned int total_index_count= 0;
	...
	{
		const mf_ModelHeader* header= (const mf_ModelHeader*) mf_Models::powerups_models[i];
		total_vertex_count+= header->vertex_count;
		total_index_count+= header->trialgle_count * 3;
	}

	powerups_data_.vbo.VertexData( NULL, total_vertex_count * sizeof(mf_DrawingModelVertex), sizeof(mf_DrawingModelVertex) );
	powerups_data_.vbo.IndexData( NULL, total_index_count * sizeof(unsigned short) );

	mf_DrawingModelVertex vert;
	unsigned int shift;
	shift= (char*)vert.pos - (char*)&vert;
	powerups_data_.vbo.VertexAttrib( 0, 3, GL_FLOAT, false, shift );
	shift= (char*)vert.normal - (char*)&vert;
	powerups_data_.vbo.VertexAttrib( 1, 3, GL_FLOAT, false, shift );
	shift= (char*)vert.tex_coord - (char*)&vert;
	powerups_data_.vbo.VertexAttrib( 2, 2, GL_FLOAT, false, shift );
	...
	for (unsigned int i= 0; i< mf_Powerup::LastType; i++ )
	{
		model.LoadFromMFMD( mf_Models::powerups_models[i] );

		powerups_data_.vbo.VertexSubData(
			model.GetVertexData(), 
			model.VertexCount() * sizeof(mf_DrawingModelVertex),
			vertex_shift * sizeof(mf_DrawingModelVertex) );
		...
		powerups_data_.vbo.IndexSubData(
			model.GetIndexData(),
			model.IndexCount() * sizeof(unsigned short),
			index_shift * sizeof(unsigned short) );

		powerups_data_.models[i].first_index= index_shift;
		powerups_data_.models[i].index_count= model.IndexCount();
...
void mf_Renderer::DrawSky(  bool draw_to_water_framebuffer )
{
	...
	*/
	if( settings_.clouds_intensity != mf_Settings::CloudsDisabled )
	{
		...
		Mat4Scale( scale_mat, g_clouds_distance_scaler * GetSceneRadius() );
		Mat4Mul( scale_mat, translate_mat, mat );
		Mat4Mul( mat, view_matrix_ );

		sky_clouds_data_.clouds_shader.UniformMat4( "mat", mat );
		...
		sky_clouds_data_.clouds_dome_vbo.Bind();
		glDrawElements(
			GL_TRIANGLES,
			sky_clouds_data_.clouds_dome_vbo.IndexDataSize() / sizeof(unsigned short),
			GL_UNSIGNED_SHORT,
			0 );
...
void mf_Renderer::DrawPowerups()
{
	...
	for( unsigned int i= 0; i< game_logic_->GetPowerupCount(); i++ )
	{
		...
		float dist_to_aircraft= Distance( powerups[i].pos, player_->GetAircraft()->Pos() );
		float alpha= 1.0f;
		if( dist_to_aircraft < 64.0f ) alpha = dist_to_aircraft / 64.0f;
		if( alpha < 0.2f ) alpha= 0.2f;
		powerups_data_.shader.UniformFloat( "trans", alpha );

		glDrawElements( GL_TRIANGLES,
			powerups_data_.models[ powerups[i].type ].index_count,
			GL_UNSIGNED_SHORT,
			(void*) ( powerups_data_.models[ powerups[i].type ].first_index * sizeof(unsigned short) ) );
	}
	glDisable( GL_CULL_FACE );
	glDisable( GL_BLEND );
}
